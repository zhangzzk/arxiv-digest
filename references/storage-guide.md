...
4. Fallback: `~/.claude/arxiv-digest`

## Automatic Initialization

The skill automatically:
1. Creates `~/.claude/arxiv-digest/` on first run if it doesn't exist
...
Update the `build_profile.py` script to save directly to `~/.claude/arxiv-digest/`:

```bash
python3 scripts/build_profile.py \
    --name "Your Name" \
    --storage-dir ~/.claude/arxiv-digest
```
...
mkdir -p ~/.claude/arxiv-digest

# Copy existing files
cp /mnt/user-data/uploads/researcher_profile.json ~/.claude/arxiv-digest/
cp /mnt/user-data/uploads/arxiv_preferences.json ~/.claude/arxiv-digest/
...
```bash
# On machine A
tar -czf arxiv-digest-backup.tar.gz ~/.claude/arxiv-digest
...
- Delete `~/.claude/arxiv-digest/` to reset completely
...
### Permission errors
```bash
chmod 755 ~/.claude/arxiv-digest
chmod 644 ~/.claude/arxiv-digest/*.json
```

### Starting fresh
```bash
rm -rf ~/.claude/arxiv-digest
```
